...
export default async function Home() {
  ...
  return <main className="page">
    <header>
      ...
      <p>관측 시각과 예측 표본을 확인할 수 있어요. 30분 넘게 지난 관측은 현재 상태로 해석하지 않습니다.</p>
    </header>
    ...
    <div className="place-list">
      {overview.rows.map(({ place, snapshot, quality }) => <section key={place.id} className="panel place-panel" aria-labelledby={`place-${place.id}`}>
        ...
        {!snapshot || !quality ? <p role="status">현재 자료를 가져오지 못했어요. 다른 공원의 정보는 계속 볼 수 있습니다. 잠시 후 다시 확인해주세요.</p> : <>
          ...
          {quality.freshness !== "fresh" && <p className="notice">{quality.freshness === "stale" ? "1시간 넘게 지난 자료입니다." : "30분 넘게 지난 자료입니다."} 현재 상태로 간주하지 않아요. 30~60분 전 자료의 미래 예측은 사용자가 허용한 참고 비교에만 사용합니다.</p>}
