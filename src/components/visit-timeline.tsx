...
export function VisitTimeline({ candidate, maximum }: { candidate: Candidate; maximum: CongestionLevel }) {
  ...
  return <section className="visit-timeline" aria-labelledby={`${id}-title`}>
    ...
    <div className="forecast-ruler">
      ...
      {points.map((p, index) => <div className={`forecast-sample ${points.length > 4 && index !== 0 && index !== points.length - 1 && index % Math.ceil((points.length - 1) / 3) !== 0 ? "compact-sample" : ""}`} key={p.id} style={{ left: `${(Date.parse(p.at) - firstMs) / 60_000 / span * 100}%` }}><time dateTime={p.at}>{shortTime(p.at)}</time><span className="sample-dot" /><span>{p.congestion}</span></div>)}
      <div className="stay-band" style={{ left: `${Math.max(0, Math.min(100, offset / span * 100))}%`, width: `${Math.max(0, Math.min(100 - Math.max(0, offset / span * 100), (duration ?? 0) / span * 100))}%` }}><span>{duration ? `${duration}분 체류` : "도착"}</span></div>
    </div>
    ...
    <div className={`timeline-reading ${arrival.kind === "between" ? "estimated-reading" : ""}`}>
      {valid ? <><strong>{arrival.kind === "exact" ? `${shortTime(at)} 공식 예측` : `${shortTime(at)} 전후 공식 예측`}</strong><p>{arrival.evidence.map(p => `${shortTime(p.at)} ${p.congestion}`).join(" → ")}</p>
