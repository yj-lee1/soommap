...
export function AdjustmentControls({ state, places, pending, onAdjust }: {
  ...
  return <section className="adjustment-panel" aria-labelledby="adjust-title">
    <h2 id="adjust-title">바꾸기 어려운 조건은 고정해요</h2>
    <p className="note">고정·제외는 AI 없이 다시 비교합니다. 추가 고정을 해제하면 처음 허용한 범위로 돌아갑니다.</p>
    {state.base.hard.pinnedPlaceId && <p className="pin-label">처음부터 고정한 장소: {names.get(state.base.hard.pinnedPlaceId)}. 변경하려면 위 입력의 허용 범위를 바꿔주세요.</p>}
    {state.base.hard.pinnedArrivalAt && <p className="pin-label">처음부터 고정한 시각: {formatSeoulTime(state.base.hard.pinnedArrivalAt)}. 변경하려면 위 입력의 도착 범위를 바꿔주세요.</p>}
    ...
    <details><summary>말로 짧게 정정하기</summary><form onSubmit={correct}>
