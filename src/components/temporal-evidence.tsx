...
import type { Candidate, CongestionLevel, ForecastTrend, VisitAssessment } from "@/lib/domain/types";
...
export function TemporalEvidence({ candidate, maximum = "보통" }: { candidate: Candidate; maximum?: CongestionLevel }) {
  const { arrival, visit } = candidate, population = arrival.population;
  return <div className="temporal-evidence">
    <p className="preference-label" data-state={visit.preference}>{preferenceText[visit.preference]}</p>
    {visit.coverage !== "complete" && <p className="notice">전체 체류 구간을 평가할 수 없어 추천에서는 제외했어요.</p>}
    <VisitTimeline key={candidate.id} candidate={candidate} maximum={maximum} />
    <details><summary>혼잡 단계별 그래프 펼치기</summary>
