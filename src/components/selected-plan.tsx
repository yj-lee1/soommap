...
export function SelectedPlanPanel({ plan, choice, conditions, places, origin, pending, error, onConfirm, onRecheck, onAdjust }: {
  ...
  useEffect(() => { const interval = setInterval(() => setNow(Date.now()), 15_000); return () => clearInterval(interval); }, []);
  useEffect(() => { heading.current?.focus({ preventScroll: true }); heading.current?.scrollIntoView({ block: "start" }); }, [plan?.confirmedAt]);
  ...
  return <section className={`panel selected-plan ${plan?.confirmedAt ? "confirmed-plan" : ""}`} aria-labelledby="selected-title" aria-busy={pending}>
    <h2 ref={heading} tabIndex={-1} id="selected-title">{plan?.confirmedAt ? "확정한 산책 계획" : "선택한 계획 확인"}</h2>
    <div className="ticket-top"><span className="ticket-label">SOOMMAP / 오늘의 산책표</span>{plan?.confirmedAt && <span className="confirmed-stamp" aria-label="계획 확정">계획<br />확정 ✓</span>}<h3>{place.name}</h3><p>{formatSeoulTime(choice.arrivalAt)} 도착 · {conditions.originalPlan.durationMinutes ? `${conditions.originalPlan.durationMinutes}분 산책` : "체류시간 미정"}</p></div>
    ...
    {candidate && <><details><summary>선택한 계획의 혼잡 근거</summary><TemporalEvidence candidate={candidate} maximum={conditions.soft.maximumPreferredCongestion} />
      ...
      <p>처음 계획 대비: {candidate.change.placeChanged === null ? "처음 정한 장소" : candidate.change.placeChanged ? "장소 변경" : "장소 유지"} · {candidate.change.arrivalDeltaMinutes === null ? "처음 정한 도착시각" : candidate.change.arrivalDeltaMinutes === 0 ? "도착시각 유지" : `${Math.abs(candidate.change.arrivalDeltaMinutes)}분 ${candidate.change.arrivalDeltaMinutes > 0 ? "늦춤" : "앞당김"}`}</p></details>
    ...
    {plan?.confirmedAt && <><h4 className="execution-heading">이제, 나가볼까요? ↗</h4><p>계획을 확정했어요. {candidate?.travel ? `대중교통 약 ${Math.ceil(candidate.travel.totalSeconds / 60)}분이 반영됐어요.` : "이동시간은 반영하지 않은 계획입니다."}</p>{origin ? <NavigationActions origin={origin} place={place} /> : <p className="note">위에서 출발지를 선택한 뒤 비교하면 확정 후 출발·도착지가 입력된 길찾기를 이용할 수 있어요.</p>}<div className="action-row">
    ...
    <details className="selection-footnote"><summary>자료 재확인·저장 안내</summary><p className="note">자료를 다시 확인해도 선택한 장소·시각은 자동으로 바뀌지 않아요. 화면의 자료는 공용 캐시를 사용하며 최대 5분 간격으로 갱신합니다. 선택·확정 계획은 이 브라우저에 저장되며, 다시 열 때 이전 혼잡·이동시간은 최신 정보로 복원하지 않습니다.</p></details>
  </section>;
}
