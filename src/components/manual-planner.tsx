...
export function ManualPlanner({ overview, mobilityReady }: { overview: PopulationOverview; mobilityReady: boolean }) {
  ...
  return <>
    ...
    {!inputOpen && <div className="input-summary"><span className="summary-tab">나의 산책 메모</span><h2>이 마음은 그대로.</h2><p>{text || "직접 정한 조건으로 비교했어요."}</p>{applied && <div className="original-reference"><span>처음 계획</span><strong>{applied.originalPlan.placeId ? names.get(applied.originalPlan.placeId) : "목적지 미정"}</strong><p>{applied.originalPlan.preferredArrivalAt ? formatSeoulTime(applied.originalPlan.preferredArrivalAt) : "도착시각 미정"} · {applied.originalPlan.durationMinutes ? `${applied.originalPlan.durationMinutes}분 산책` : "체류시간 미정"}</p></div>}<p className="note">{origin?.name || "출발지 미정"} · {automatic ? "지금 출발" : "방문 시각 직접 지정"}</p><p className="note">고정·제외 조건은 비교 결과에서 조정해요.</p><ParkMap places={places} activeId={choice?.placeId ?? result?.options.find(o => o.role === "recommended")?.candidate?.placeId} compact /></div>}
