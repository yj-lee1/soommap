...
export function OriginControls({ origin, automatic, transit, disabled, mobilityReady, onOrigin, onMode, onReset }: {
  ...
  return <section className="origin-panel" aria-labelledby="origin-title">
    ...
    {!mobilityReady && <p className="notice">이동시간 연결을 사용할 수 없어요. 지금은 도착시각을 직접 정해 비교한 뒤, 선택한 출발지로 지도 길찾기를 이용할 수 있어요.</p>}
    ...
    <details><summary>위치 사용·출발역 자료 안내</summary><p className="note">출발 좌표는 이동시간 계산 시 TMAP, 확정 후 길찾기를 누를 때 선택한 지도 서비스에 전달돼요. AI 조건 해석에는 출발 좌표를 전달하지 않습니다. 검색 역은 <a href="https://data.seoul.go.kr/dataList/OA-21232/S/1/datasetView.do" target="_blank" rel="noreferrer">서울 열린데이터광장 역사마스터</a> (2026.9.12 기준)를 사용해요.</p></details>
  </section>;
}
