...
후속 갱신: 사용자가 이미지 직접 업로드를 선택해 대표 PNG와 실제 캡처 4장을 채팅으로 전달했다. 확장 권한/잠금 해제 응답은 더 기다리지 않는다. 제목·문제·AI 설명은 최신 `docs/submission/wanted-draft.md`의 문구로 교체하고 임시저장/새로고침 검증을 완료했다. 이미지 웹 업로드/최종 제출을 완료했다고 기록하지 않는다.
